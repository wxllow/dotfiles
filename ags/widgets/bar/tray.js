  ...
  const trayContent = Box({
    ...
    attribute: {
      ...
      onAdded: (box, id) => {
        ...
        box.add(widget);
        box.show_all();
        if (box.attribute.items.size === 1) trayRevealer.revealChild = true;
      },
      onRemoved: (box, id) => {
        ...
        if (box.attribute.items.size === 0) trayRevealer.revealChild = true;
      },
    },
    setup: (self) => {
      self
        .hook(
          SystemTray,
          (box, id) => {
            console.log(`Added item ${id}: ${box}`);
            box.attribute.onAdded(box, id);
          },
          "added"
        )
        .hook(
      ...
      SystemTray.items.forEach((item) => {
        self.attribute.onAdded(self, item);
      });
    },
  });
