-- this has some modifications btw
...
local function musixmatch_dl()
    print(mp.get_property_native('metadata'))
...
mp.add_key_binding('Alt+m', 'musixmatch-download', musixmatch_dl)
...
-- credits to https://github.com/CogentRedTester/mpv-coverart
...
local function auto_musixmatch_dl()
    if not mp.get_property('current-tracks/sub/external-filename') and is_audio_file() then
        musixmatch_dl()
    end
end

-- runs automatically whenever a file is loaded
mp.register_event('file-loaded', auto_musixmatch_dl)
